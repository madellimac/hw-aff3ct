...
def compare_char_encoder(data_in: List[chr]):
    ...
    for chr in data_in:
        # My encoder
        encoded_bits.append(my_encoder.encode_char(chr))
    ...
    for i in range(len(encoded_bits_vhdl)):
        if(encoded_bits_vhdl[i] != encoded_bits[i]):
            print("error")
            print("vhdl index: " + i + " | bit value: " + encoded_bits_vhdl[i])
            print("My encoder index : " + i + " | bit value: " + encoded_bits[i])
            break


if __name__ == "__main__":
    img_array = vhdl_encoder.read_image_grayscale("images/baboon.jpg")
