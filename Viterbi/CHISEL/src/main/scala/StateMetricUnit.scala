...
class StateMetricUnit( param : Viterbi_Param) extends Module {
    ...
    val half_max = vACS.map(_.o_SM(param.Q_SM-2)).reduce(_ & _)

    for(i <- 0 until (param.N_states/2)) {
        when(half_max === 1.U){
            vACS(i).i_SM(0) := vACS(2*i  ).o_SM - (1<<param.Q_SM-2).U
            vACS(i).i_SM(1) := vACS(2*i+1).o_SM - (1<<param.Q_SM-2).U

            vACS(i+(param.N_states/2)).i_SM(0) := vACS(2*i  ).o_SM - (1<<param.Q_SM-2).U
            vACS(i+(param.N_states/2)).i_SM(1) := vACS(2*i+1).o_SM -(1<<param.Q_SM-2).U
        }.otherwise {
