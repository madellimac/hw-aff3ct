...
class ComChain(pFrameSize: Int, pDataWidth: Int) extends Module {
    val io = IO(new Bundle {
        val i_dv = Input(Bool())
        val i_data = Input(UInt(pDataWidth.W))
        val i_ready = Input(Bool())
        val o_dv = Output(Bool())
        val o_data = Output(UInt(pDataWidth.W))
...
object ComChainMain extends App {
  val stage = new ChiselStage
  stage.execute(
    Array(
    "-X", "verilog", 
    "-e", "verilog", 
    "--target-dir", "../VERILOG/generated"), 
  Seq(ChiselGeneratorAnnotation(() => new ComChain(20, 8)))
  )
}
